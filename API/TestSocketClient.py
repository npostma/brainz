...
for(i) in range(0, 2000):
    ...
    socket.sendall(
        str("{\"command\": \"learn\", \"input\": [1, 1, 1, 1, 1, 1], \"expectedOutput\": [1, 1]}" + '\0').encode())
    socket.sendall(
        str("{\"command\": \"learn\", \"input\": [0, 0, 0, 0, 0, 0], \"expectedOutput\": [0, 0]}" + '\0').encode())
    socket.sendall(
        str("{\"command\": \"learn\", \"input\": [0, 0, 0, 1, 1, 1], \"expectedOutput\": [0, 1]}" + '\0').encode())
    socket.sendall(
        str("{\"command\": \"learn\", \"input\": [1, 1, 1, 0, 0, 0], \"expectedOutput\": [1, 0]}" + '\0').encode())
